function [x,r,g,info,pvalue, dvalue, dGap] = spgl1Exp( A, b, tau, sigma, x, options)
% spgl1Exp.m: update: update B at every iter, but run dual every exp-iter
% spgl1Copy.m: update June 8: have rGap and dual mode + dnopt with tol and
% iter limit
...
while 1 
    ...
    %-----------------------------------------------------------------
    % Circular buffer to update the base set in B (not solve dual yet)
    %-----------------------------------------------------------------
    ...
    if dualFlag == 1 && (iter == baseIter + expValue)
        ...
        [ydual,Time] = solver(solverNum,A,b,p,Q,tau,varargout{1:7});
